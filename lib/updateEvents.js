...
function mainUpdate(req) {
    ...
    if (active_event) {
        const d = new Date(active_event);
        const u = new Date();
        if (u > d) {
            valErrors.push({ field: 'active_event', message: ' date is less than today' });
        } else {
            valUpdate.push({ active_event: active_event });
        }
    };
    if (description) {
    ...
    if (valErrors.length === 0) {
        const resultado = valUpdate.reduce(function (result, item) {
